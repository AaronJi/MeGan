...
Our data adheres to the `openai` format, which is a specific case of the `sharegpt` format in LlamaFactory. More details can refer to [data/README.md](data/README.md). 

To accommodate the newly-added condition in the data sample, we add a new type of `style_tag` to `DatasetAttr` in the [parser.py](src/llamafactory/data/parser.py). There might also be attributes of `history` or `context` in `DatasetAttr` depending on the detailed condition or dataset type. The sample is then processed by the `convert_sharegpt` function in [aligner.py](src/llamafactory/data/aligner.py). 
