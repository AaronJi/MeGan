...
class LlamaRotaryEmbedding(nn.Module):
    def __init__(
        ...
        inv_freq, self.attention_scaling = self.rope_init_fn(self.config, device, **self.rope_kwargs) # rope_scaling.factor=32?
    ...
    def forward(self, x, style=None):
        if self.config.pretraining_tp > 1:
            ...
            if style is not None:
                ...
                beta = self.beta_generator(style_aligned)
            else:
                beta = 0
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 传递style参数给MLP
        hidden_states, beta = self.mlp(hidden_states, style=style)
        hidden_states = residual + hidden_states
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if cache_position is None:
            ...
            cache_position = torch.arange(past_seen_tokens, past_seen_tokens + inputs_embeds.shape[1], device=inputs_embeds.device) # [0, 1, ..., L-1]
        if position_ids is None:
            position_ids = cache_position.unsqueeze(0) # [[0, 1, ..., L-1]]

        causal_mask = self._update_causal_mask(attention_mask, inputs_embeds, cache_position, past_key_values, output_attentions)  # [B, L] => [B, 1, L, L]
