...
import torch
from transformers import AutoConfig, AutoModelForCausalLM, AutoModelForVision2Seq, AutoProcessor, AutoTokenizer, LlamaTokenizer
...
def load_config(model_args: "ModelArguments", is_meta=False) -> "PretrainedConfig":
    r"""
    Loads model config.
    """
    init_kwargs = _get_init_kwargs(model_args)
    #if is_meta:
    #    config = LlamaConfig(**init_kwargs)
    #    return config
    #else:
...
def load_model(
    ...
    """
    is_meta = finetuning_args.finetuning_type in ["meta_swiglu", "meta_swiglu_simple", "meta_swiglu_full"]

    init_kwargs = _get_init_kwargs(model_args)
    config = load_config(model_args, is_meta)
    ...
    if model is None and not lazy_load:
        ...
        if model_args.mixture_of_depths == "load":
            ...
            #model.meta_swishglu_attn_key = model_args.meta_swishglu_attn_key
            #model.meta_configs["meta_swishglu_attn_key"] = model_args.meta_swishglu_attn_key
            #model.set_meta_configs("meta_swishglu_attn_key", model_args.meta_swishglu_attn_key)
