...
def convert_sharegpt(
    ...
    for turn_idx, message in enumerate(messages):
        ...
        if message[dataset_attr.role_tag] in [dataset_attr.user_tag, dataset_attr.assistant_tag]:
    ...
    if dataset_attr.kto_tag and isinstance(example[dataset_attr.kto_tag], bool):  # kto example
        ...
        if "shakespeare_qa" in dataset_attr.dataset_name or "GYAFC_qa" in dataset_attr.dataset_name or "mic/qa" in dataset_attr.dataset_name:
            ...
            prompt, response, style = align_style_role(messages, dataset_attr, aligned_messages)
        else:
            # normal example
            style = None
            prompt = aligned_messages[:-1]
...
def align_style_qa(messages, dataset_attr, aligned_messages):
...
def align_style_role_qa(messages, dataset_attr, aligned_messages):
    style = None
    # print(dataset_attr.style_tag)
    for message in messages:
        # print(message)
        if message[dataset_attr.role_tag] == dataset_attr.style_tag:
            style = message[dataset_attr.content_tag]
            if style is None:
                style = "None"
            # print(style)
            break
...
def align_style_role(messages, dataset_attr, aligned_messages):
    style = None
    # print(dataset_attr.style_tag)
    for message in messages:
        # print(message)
...
def convert_chatgpt(examples: Dict[str, List[Any]], dataset_attr: "DatasetAttr", data_args: "DataArguments") -> Dict[
    ...
    convert_images = partial(_convert_images, dataset_attr=dataset_attr, data_args=data_args)
    tag_mapping = {dataset_attr.user_tag: Role.USER.value, dataset_attr.assistant_tag: Role.ASSISTANT.value,
        dataset_attr.observation_tag: Role.OBSERVATION.value, dataset_attr.function_tag: Role.FUNCTION.value,
        dataset_attr.system_tag: Role.SYSTEM.value, }
    ...
    for i, messages in enumerate(examples[dataset_attr.messages]):
        ...
        if dataset_attr.kto_tag and isinstance(examples[dataset_attr.kto_tag][i], bool):  # kto example
            ...
            if examples[dataset_attr.kto_tag][i]:
                ...
                response = [{"role": Role.ASSISTANT.value, "content": ""}] + response
        elif (dataset_attr.ranking and isinstance(examples[dataset_attr.chosen][i], dict) and isinstance(
            examples[dataset_attr.rejected][i], dict)):  # pairwise example
            chosen = examples[dataset_attr.chosen][i]
            rejected = examples[dataset_attr.rejected][i]
            if (chosen[dataset_attr.role_tag] not in accept_tags[-1] or rected[dataset_attr.role_tag] not in
                    accept_tags[-1]):
        ...
        if dataset_attr.tools:
            udf_colunms = dataset_attr.tools.split(",")
            udf_column_dict = {}
            for the_col in udf_colunms:
                udf_column_dict[the_col] = examples[the_col][i]
            outputs["tools"].append(json.dumps(udf_column_dict, ensure_ascii=False))
        else:
