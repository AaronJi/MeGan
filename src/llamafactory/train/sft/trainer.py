...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(
        ...
        if self.finetuning_args.finetuning_type in ["meta_swiglu", "meta_swiglu_simple", "meta_swiglu_full"]:
            if self.finetuning_args.meta_l2_lambda > 1e-10:
                        ...
                        if isinstance(module, nn.Sequential):
                            for element in module:
                                if isinstance(element, nn.Linear):
                                    regularization_loss += torch.norm(element.weight)
                        #elif isinstance(module, nn.Linear):
                        #    regularization_loss += torch.norm(module.weight)
                loss += self.finetuning_args.meta_l2_lambda * regularization_loss
